...
// CORS — handle OPTIONS preflight FIRST, before any other middleware.
// This is critical for Edge/Firefox which send fresh preflights every request.
// Chrome caches preflights so it works without this — Edge does not.
app.options('*', (req, res) => {
  res.set({
...
// Serve frontend static files from same directory as server.js
app.use(express.static(path.join(__dirname), {
  index: false, // never auto-serve index.html — we handle it in the SPA fallback
  setHeaders(res, filePath) {
    // Allow Edge to cache static assets properly
...
// Must be LAST. Only catches GET — never intercepts POST/PATCH/DELETE to /api/*.
app.get('*', (req, res) => {
